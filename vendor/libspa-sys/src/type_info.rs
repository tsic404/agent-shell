// NOTE (agent-shell vendor patch): `src/type-info.c` guards the `spa_type_*`
// symbols introduced after pipewire 0.3.15 (see the `PW_CHECK_VERSION` blocks)
// so this crate builds against older system pipewire (e.g. UOS 20, pipewire
// 0.3.15). The `extern` declarations below are intentionally left unguarded:
// an unused `extern` static produces no link-time symbol reference, and
// `libspa` 0.10.1 only references the symbols available since pipewire 0.3.x
// (`spa_types`, `spa_type_param`, `spa_type_media_type`, ...). Removing the
// declarations would couple this file to the system version without benefit.
