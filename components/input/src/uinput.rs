...
/// `UI_SET_PROPBIT = _IOW('U', 105, int)`。
const UI_SET_PROPBIT: libc::c_ulong = 0x4004_5569;
...
/// 注册虚拟设备：事件类型位、键位、相对/绝对位、UI_DEV_CREATE。
fn register_device(file: &std::fs::File) -> std::io::Result<()> {
    ...
    let ret = unsafe { libc::ioctl(fd, UI_DEV_CREATE, &dev as *const UinputUserDev) };
    if ret < 0 {
...
fn write_event(file: &std::fs::File, type_: u16, code: u16, value: i32) -> std::io::Result<()> {
    ...
    // SAFETY: bytes 指向 repr(C) 的 InputEvent，len 为其真实字节长度；
    // uinput 是字符设备，write 语义与文件一致。
    let written = unsafe {
        libc::write(
            file.as_raw_fd(),
    ...
    if written as usize != bytes.len() {
        return Err(std::io::Error::other("short write to uinput"));
    }
    Ok(())
}
...
impl InputEvent {
    fn as_bytes(&self) -> &[u8] {
        // SAFETY: InputEvent 是 repr(C) 的 POD，无内部指针；转字节切片仅用于
        // 逐字节写入 uinput fd。
        unsafe {
