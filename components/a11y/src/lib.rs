...
//!
//! 四层：atspi_bridge（D-Bus 桥接）→ tree（树模型）→ semantic_locator（定位）
//! → action（元素操作）。装配走 [`AtSpiComponent::probe`] 探测 `org.a11y.Bus`
//! 可达后构造组件并实现 [`A11yComponent`] 契约；不可达时 `is_available() ==
//! false`、doctor 显示 Unavailable 而非崩溃（验收标准）。
