...
//!
//! [`HyprlandCompositor`] 组合 [`WlrWaylandCompositor`] 基类（wlr 标准协议完整实现），
//! 叠加 hyprland_* 私有协议（[`wayland`]）与 hyprctl socket IPC（[`hyprctl`]/
//! [`event_socket`]）双通道。纯 Wayland 会话（无 X11 变体）；操作选择矩阵见 §9.5。
