...
//! - 工作区：`org.kde.KWin.VirtualDesktopManager` 的 `desktops`（`a(iss)`，
...
impl KWinNative {
    ...
    /// `org.kde.KWin.VirtualDesktopManager` 的 `desktops` 属性（`a(iss)`，
    /// 每项 `(position, id, name)`，按 position 升序）。
    ///
    /// 首位是有符号 `i32`（KWin XML 声明 `a(iss)`，`DBusDesktopDataStruct.position`
    /// 虽为 C++ uint 但 QtDBus 按 XML 签名以 int32 序列化）——zvariant 严格
    /// 签名匹配，用 `u32` 读会 SignatureMismatch 恒失败。`position` 由调用方
    /// 校验 `>= 0` 后转 `u32`。
    pub async fn desktops(&self) -> Result<Vec<(i32, String, String)>> {
        self.vd
            .get_property::<Vec<(i32, String, String)>>("desktops")
            .await
    ...
    /// 原生工作区列表：`desktops`（`a(iss)` → `(position, id, name)`）+ `current`
    ...
    /// inactive，误导调用方。position < 0（协议不变量之外）的工作区跳过。
    pub async fn workspaces(&self) -> Result<Vec<agent_shell_core::types::WorkspaceInfo>> {
        ...
        Ok(desktops
            .into_iter()
            .filter_map(|(position, id, name)| {
                let number = u32::try_from(position).ok()?.saturating_add(1);
                Some(WorkspaceInfo {
                    id: WorkspaceId {
                        native_id: id.clone(),
                        de_type: DesktopEnvironment::KDE,
...
mod tests {
    ...
    /// `desktops` wire 签名契约：KWin 声明 `a(iss)`（有符号 i32），用
    /// `Vec<(u32,..)>` 读取会 SignatureMismatch——锁定本修复依赖的类型选择。
    #[test]
    fn desktops_wire_signature_is_a_iss() {
        use zbus::zvariant::Type;
        assert_eq!(
            Vec::<(i32, String, String)>::SIGNATURE.to_string(),
            "a(iss)",
            "KWin VirtualDesktopManager.desktops is declared a(iss)"
        );
        assert_eq!(
            Vec::<(u32, String, String)>::SIGNATURE.to_string(),
            "a(uss)",
            "sanity: u32 position would read a(uss), not a(iss)"
        );
    }

    /// 端到端反序列化：`desktops` 的 `a(iss)` wire 数据只能反序列化为
    /// `Vec<(i32,..)>`，`Vec<(u32,..)>` 因签名不匹配（`a(uss)`）失败——
    /// 旧实现用 `u32` 读 `a(iss)` 正是 TC-004 恒失败的根因。
    #[test]
    fn a_iss_wire_data_rejects_u32_position() {
        ...
        let bytes = to_bytes(ctx, &data).expect("serialize a(iss)");

        let (back, _): (Vec<(i32, String, String)>, usize) =
            bytes.deserialize().expect("a(iss) -> Vec<(i32,..)>");
        assert_eq!(back, data);

        let err = bytes.deserialize::<(Vec<(u32, String, String)>, usize)>();
        assert!(
            err.is_err(),
            "a(iss) must NOT deserialize into Vec<(u32,..)> (signature mismatch)"
        );
    }
}
