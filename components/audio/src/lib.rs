...
//! 的公共实现与 DE 封装优先路由：pipewire（wpctl CLI）、pulseaudio（pactl CLI，
//! `@DEFAULT_SINK@` 别名屏蔽服务器差异）、[`DePriorityRouter`]（DE 专有 D-Bus 封装
//! 优先 → 公共实现降级，§21.4）。探测顺序（§4.2）：pipewire 运行 → PipeWire，
//! 否则 pulseaudio 运行 → PulseAudio，否则 None。
