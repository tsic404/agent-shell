//! Sway backend 装配器（§4.2–4.3、§3.3 装配矩阵 Sway 行）。
//!
//! 纯 Wayland 合成器（仅 Wayland）。装配清单见 §3.3：合成器 SwayCompositor
//! （WlrWayland 基类 + Sway IPC），音频/输入/截图/无障碍/剪贴板走公共探测链，
//! 电源 UPower，通知/外观/启动器 portal 三件套，系统服务 Systemd + Logind。
...
use agent_shell_compositor_sway::SwayCompositor;
...
/// Sway backend 装配器（§4.3 `SwayBackend`）。
...
impl SwayBackend {
    ...
    pub async fn assemble(&self) -> Result<ComponentRegistry> {
        // 1. 合成器：SwayCompositor（WlrWayland 基类 + Sway IPC）
        let compositor: Option<Box<dyn CompositorComponent>> = match SwayCompositor::connect().await
        {
            Ok(c) => Some(Box::new(c)),
            Err(e) => {
                tracing::warn!(error = %e, "SwayCompositor assembly failed; compositor slot None");
                None
            }
        };
        ...
        let input = assemble_input(DesktopEnvironment::Sway).await;
        let capture = CaptureDispatcher::assemble().await;
        let a11y = AtSpiComponent::probe().await;
        // Sway 纯 Wayland → wl-clipboard
...
async fn assemble_audio() -> Result<Option<Box<dyn AudioServerComponent>>> {
    let router = agent_shell_audio::router::assemble_audio_router(BackendKind::Sway).await?;
    Ok(router.map(|r| Box::new(r) as Box<dyn AudioServerComponent>))
}

async fn assemble_network() -> Option<Box<dyn NetworkComponent>> {
    match agent_shell_network::detect(BackendKind::Sway).await {
        Some((n, _)) => Some(n),
        None => {
